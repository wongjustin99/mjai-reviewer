...
impl Reviewer<'_> {
    pub fn review(&self) -> Result<Review> {
        ...
        let mut stdin = mortal
            .stdin
            .take()
            .context("failed to get stdin of backend")?;
        let stdout = mortal
            .stdout
            .take()
            .context("failed to get stdout of backend")?;
        ...
        for (i, event) in events.iter().enumerate() {
            ...
            writeln!(stdin, "{to_write}").context("failed to write to backend")?;
            if verbose {
                log!("> {to_write}");
            }
            ...
            let line = stdout_lines
                .next()
                .context("failed to read from backend: unexpected EOF")?
            ...
            let output: RawAction =
                json::from_str(&line).context("failed to parse JSON output of backend")?;
            ...
            let score = if equal_ignore_aka_consumed(&output.event, &actual) {
                1.
            } else {
        ...
        let score = (raw_score / total_reviewed as f64).powi(2);
        Ok(Review {
            total_reviewed,
            score,
