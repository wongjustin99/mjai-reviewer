...
use crate::opts::{AkochanOptions, Backend, InputOptions, MortalOptions, Options, OutputOptions};
...
fn main() -> Result<()> {
    ...
    // unwrap is safe because --backend is required when --no-review is not
    // present.
    let backend = backend.unwrap();

    if backend == Backend::Mortal && log.game_length != GameLength::Hanchan {
    ...
    let review = match backend {
        Backend::Mortal => {
            canonicalize!(mortal_exe);
            canonicalize!(mortal_cfg);
        ...
        Backend::Akochan => {
            let akochan_exe: PathBuf = [&akochan_dir, Path::new("system.exe")]
                .into_iter()
                .collect();
            canonicalize!(akochan_dir);
            canonicalize!(akochan_exe);
            canonicalize!(akochan_tactics);
    ...
    let out = if let Some(filename) = out_file {
        ...
        let mut filename = log_source
            .default_output_filename(backend, player_id)
            .into_os_string();
    ...
    //     verbose: arg_verbose,
    // };
    // let review_result = review(review_args).context("failed to review")?;
